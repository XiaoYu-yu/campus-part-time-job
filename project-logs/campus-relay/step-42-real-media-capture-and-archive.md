...
Step 43 已选择路径 B：不补固定 after-sale 样本，不继续补媒体，正式收住媒体线。

理由：

1. Step 42 媒体已经覆盖当前主链路和两个核心 admin 只读页。
2. after-sale 固定样本不是当前答辩或交接的硬阻塞。
3. 继续补样本会把交付收尾拖回媒体循环，收益低于进入下一条非 bridge 后端评估线。
4. 如果后续答辩明确要求真实 after-sale 执行记录，可以单独打开 targeted after-sale 样本补采，不需要重录主链路。
