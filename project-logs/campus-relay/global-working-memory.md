...
10. 当前下一优先产品问题是：用户端已有结果回看与入驻入口，但仍缺“校园代送下单 / 我的代送单”的移动端最小主入口

优先级建议：

1. 如果近期仍是 owner 自测，优先补用户端校园代送下单 / 我的代送单最小入口
2. 如果继续 Android 线，同步评估真机 / 服务器 API base 分层，而不是继续抠 admin 样式
