...
1. Step 147 已完成内测入口状态复盘：
   - 本地 `main` 与 GitHub `origin/main` 一致，均为 `f06d837`。
   - 服务器运行态 remote smoke 25 项通过、0 项失败、0 项跳过。
   - key SSH 与 Docker LogConfig 只读检查通过。
   - 当前结论：本地 / 内测型试运营可用，但不是完整产品级正式上线。
2. Step 148 建议优先补“内测用户试用说明与账号发放边界”：
   - 谁可以参与内测。
   - admin / customer / parttime 三类账号如何发放。
   - 当前可以测哪些链路。
   - 当前不要测哪些能力。
   - 反馈如何记录。
3. 当前仍未处理：
