...
1. Step 148 已完成内测用户试用说明与账号发放边界：
   - 新增 `docs/deployment/internal-trial-user-test-guide.md`。
   - 明确 owner-controlled 内测参与范围。
   - 明确 admin / customer / parttime 三类账号发放边界。
   - 明确各端可测与不要测范围。
   - 明确反馈记录格式和测试前检查。
2. Step 149 建议进入“内测反馈记录模板 / issue triage 规则”：
   - 给用户反馈建立最小模板。
   - 明确阻塞 / 主要 / 次要 / 建议四类优先级。
   - 明确哪些反馈进入修复，哪些进入后续规划。
   - 不默认改业务代码，先把反馈入口和分级规则固定下来。
3. 当前仍未处理：
