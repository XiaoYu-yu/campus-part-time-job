...
14. Step 83 建议进入“试运营运维化最小能力设计与实现边界收敛”：
   - 优先评估并落到一组最小本地运维脚本或命令入口，例如启动、preflight、H2 重置、样本校验。
   - 目标是把现有 trial-operation 文档步骤压缩成更可执行、可复用、可交接的本地试运营操作层。
   - 仍不重开 bridge，不继续做页面 polish，不默认继续扩非 bridge 业务功能。
