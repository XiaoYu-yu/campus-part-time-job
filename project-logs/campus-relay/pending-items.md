...
2. 在不动旧系统的前提下，评估第三个 admin 最小只读运营页或现有 admin 演示页的细化需求
3. 视业务需要为售后执行、异常上报补更细粒度的历史审计能力
4. 视业务需要为 settlement 补更完整的批次复核、撤回和对账能力
5. 继续保持旧系统可运行，不在 Step 10 贸然切前端主链路
...
- frontend 已打通 admin settlement 批次列表页、批次详情页和售后执行分页页最小演示入口
...
### 2. 前端 admin 侧只完成两个最小演示页

- 影响：admin 现在已有 settlement 批次演示页和售后执行演示页，但更多运营视图仍未接入
- 默认处理：Step 10 继续走新增页面或新增轻量脚本，不替换旧主链路
