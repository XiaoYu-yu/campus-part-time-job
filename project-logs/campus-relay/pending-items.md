...
36. Step 71 最高优先级建议：
   - 转入整体维护 / 交付口径复盘，统一判断：
     - 当前最小闭环是否已经达到“可试运营、可演示、可交接”的稳定状态
     - 现有冻结态主线是否需要继续补文档、验证或交付说明
     - 是否还有必须补的高优先级缺口，而不是继续机械扩功能
   - 若整体口径已足够稳定，再评估是否需要进入真正产品化方向的下一阶段规划。
37. Step 71 明确禁止：
   - 不改 bridge
   - 不改 `request.js`
   ...
   - 不改后端接口
38. admin 剩余只读运营页和 Profile 页仍属于后续展示级优化候选，但默认不再继续机械 polish。
39. 第五个 admin 页继续后置，不再以“补页数”为目标。
