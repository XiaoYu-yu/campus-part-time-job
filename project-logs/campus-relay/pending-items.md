...
2. 本轮未改动：
   ...
   - 未删除旧外卖兼容模块。
3. 下一轮最高优先级：
   - A. 整理内测分发材料：APK 安装说明、测试账号说明、已知限制、反馈模板。
   - B. 做 Android 双端真机小回归：用户端发单 / 查单 / 确认，兼职端接单 / 取餐 / 送达 / 异常，后台查看订单 / 异常 / 售后 / 结算。
   - C. 若扩大到非 owner 用户，补 release 签名包和最低限度隐私 / 安全说明。
4. 继续禁止：
   - 不提交真实密钥、公网地址、服务器密码、GitHub token、腾讯地图 key 或 `.env` 内容。
   - 不重开 bridge。
   - 不改鉴权、token 附着逻辑或 `request.js`。
