...
## Step 172 协作记录 - 旧前端入口移除后的本地 smoke 复核

### 本轮目标

验证 Step 171 前端去旧后，admin、用户端、兼职端核心入口没有被误伤。

### 实际改动

- 新增 Step 172 日志。
- 新增本地 API + SPA shell smoke 报告。
- 保存 7 个浏览器截图 smoke 留痕。
- 修复 `customer_user_info` malformed JSON 导致 customer store 初始化报错的稳健性问题。
- 更新 summary / pending / file-change-list / working memory。
...
- 本地 backend `test` profile 可启动。
- 本地 frontend dev server 可启动。
- API + SPA shell smoke：25 PASS / 0 FAIL / 0 SKIP。
- 浏览器截图 smoke：7 PASS / 0 FAIL。
- smoke 后已关闭本轮启动的 backend / frontend 进程。

### 风险

- 浏览器截图 smoke 耗时偏长，后续可拆分优化。
- 后端旧模块仍未审计删除。
- release 签名包仍未完成。

### 下一轮建议

二选一：继续旧后端模块删除前依赖审计，或回到真实 release keystore / release APK 收口。

## Step 173 协作记录 - admin 反馈运营闭环与 CI 回归加固

### 本轮目标

让 App 内反馈具备 admin 可查看、可筛选、可处理、可审计的最小运营闭环，并把已稳定的测试纳入 CI。

### 实际改动

- 新增 admin 反馈分页、详情、处理接口。
- 新增反馈处理审计字段和 V14 migration。
- 新增 `/campus/feedback` 后台页面。
- 新增反馈集成测试。
- 修复共享 H2 动态数据导致的脆弱接单测试断言。
- CI 升级为 backend test + frontend lint/test/build。
- 同步 API、数据库、发布缺口和主线日志。
...
- 未改核心订单状态机。
- 未删除旧后端外卖模块或旧表。
- 未生成、读取或提交真实 release keystore、密码、证书私钥、服务器凭据或 `.env`。

### 验证结果

- backend：58 tests 全部通过。
- frontend：lint 通过，5 tests 通过，Web build 通过。
- Android public：用户端和兼职端构建均通过。
- sample validation：0 hard failures，3 个既有 optional warnings；feedback schema / seed 检查通过。

### 下一轮建议

优先由 owner 在私有环境完成 release 签名包；随后部署 V14、跑远端 smoke，并把 admin 反馈页加入浏览器 smoke。

## Step 174 协作记录 - 反馈防刷、部署隔离预案与 MuMu 回归

### 本轮目标

补齐公开反馈的基础防刷与入口安全，并使用 owner 已有的 MuMu 完成最新 Android 包真实回归。

### 实际改动

- 增加反馈 60 秒重复提交防护。
- 增加 Nginx 反馈限流、TLS 与安全头。
- 新增独立 Podman 单机部署预案，但未执行部署。
- 修复 WebView smoke 的协议勾选和参数透传。
- 新增 MuMu 使用说明。
- 保存 MuMu 双端首屏、业务页、JSON 报告和 APK 哈希清单。
- 更新 Step 174 日志与主线状态。
...
- 未改核心订单状态机。
- 未删除旧后端模块或旧表。
- 未生成、读取或提交真实 release keystore。
- owner 要求停止 Linux 后未再连接服务器。

### 验证结果

- MuMu Android 12 在线，`10.0.2.2` 可访问宿主机。
- 双端 emulator Debug APK 构建、安装、启动和截图通过。
- 用户端 WebView 创建代送单并模拟支付成功。
- 兼职端 WebView 登录、资料、审核状态和可接订单查询通过。
- backend 58 tests 全绿。
- frontend lint、5 tests、build 全绿。
- sample validation 0 hard failures。

### 风险

- 正式发布仍依赖 owner 私有 release 签名材料。
- 当前 MuMu 验证的是 emulator Debug 包，不等于正式 release 包验收。
- 当前 Linux 未部署本轮代码，V14 也未在服务器执行。
- 反馈限流在多实例环境仍需共享状态方案。

### 下一轮建议

owner 私下完成 release keystore 和签名 APK，再用 MuMu / 真机跑 HTTPS release 回归；服务器操作需重新明确授权。

## Step 175 协作记录 - Android release candidate 管线与 MuMu release 验证

### 本轮目标

在 owner 明确停止 Linux 操作后，改走本地 Android release candidate 路线：补齐 release 构建、签名校验、CI 边界、手动 signed workflow，并用 MuMu 做 release 包启动级验证。

### 实际改动

- 新增 Capacitor 场景配置脚本，提交态保持 release-safe。
- 双端 `cap:sync*` 脚本按 default / emulator / lan / public 切换 HTTP cleartext 或 HTTPS-only。
- 双端 Gradle 支持版本号注入和强制 release 签名。
- 双端 release 构建校验 `key.properties` 完整性和 keystore 文件存在性。
- 双端 manifest 禁用 Android backup。
- 新增本地 release APK / AAB 构建脚本。
- 新增 release WebView 启动 smoke 脚本。
- 新增手动 signed release GitHub Actions workflow。
- 最小 CI 新增 unsigned Android release candidate job。
- 更新 release 缺口、CI 边界、移动端 README 和项目主线日志。
...
- 未部署服务器。
- 未修改集群环境。
- 未提交真实 release keystore。
- 未提交真实 `android/key.properties`。
- 未改 bridge。
- 未改 `request.js`。
- 未改 token 附着逻辑或鉴权规则。
- 未改核心订单状态机。
...
- 双端 release manifest 均为 no-cleartext / no-backup。
- 双端生成的 Capacitor 配置均为 `androidScheme=https`、`cleartext=false`。
- 双端 signed release APK 通过 `apksigner verify`。
- 双端 release APK 安装到 MuMu Android 12 后，`versionCode=175`、`versionName=0.9.0-rc.175`。
- MuMu release smoke 通过：启动、焦点、截图非空、无 cleartext / network fatal 错误。
- backend 58 tests、frontend lint / 5 tests / build、sample validation CI wrapper、脚本语法检查和 `git diff --check` 均通过。
- 临时 QA keystore 和本地 `key.properties` 已删除。

### 风险

- 临时 QA 签名包不是生产签名包，不能公开分发。
- 生产 keystore / GitHub signing secrets 仍需 owner 私下完成。
- `https://xiaoyu.xin/api/campus/public/health` 从本机访问超时，需要 owner 重新授权服务器操作后再定位。
- Step 173/174/175 尚未部署到服务器，V14 migration 尚未在服务器执行。

### 下一轮建议

owner 生成生产 release keystore 或配置 GitHub Secrets 后，跑一次正式 signed release 构建；随后用 MuMu / 真机跑生产签名包主链路 smoke。服务器侧等 owner 重新明确授权后再部署和远端复核。

## Step 176 协作记录 - 138 磁盘扩容与 standalone H2 smoke 部署

### 本轮目标

在 owner 明确授权可使用 `192.168.121.138` 的前提下，完成磁盘扩容、独立 standalone 部署验证，并继续避免触碰既有集群环境。

### 实际改动

- 在 138 上扩展 `/dev/sda2`、LVM root LV 和 xfs 文件系统。
- 将当前源码同步到 `/opt/campus-part-time-job-standalone`。
- 生成私有 `.env`，权限 `0600`，未写入仓库。
- 由于 MySQL 镜像拉取受网络影响，先启动 H2 smoke-only backend / frontend。
- standalone Dockerfile / Podman 脚本增加镜像源、Maven mirror、npm mirror 支持。
- 新增 `deploy-h2-smoke.sh`，把本轮 H2 fallback 固化为可复现脚本。
- 将容器启动脚本中的敏感环境变量传递改为按名称继承，减少命令行明文泄露风险。
- 新增 Step 176 日志和远端 smoke 报告。

### 未改动内容

- 未修改集群环境。
- 未停止、修改或复用宿主机已有 MySQL。
- 未启动 standalone MySQL 容器。
- 未暴露 backend `18081` 到非本机地址。
- 未提交 `.env`、密码、证书私钥、服务器凭据、release keystore、GitHub token 或腾讯地图 key。
- 未改 bridge、`request.js`、token 附着逻辑、鉴权规则或核心订单状态机。

### 验证结果

- 138 root 文件系统扩展到约 37G，`/` 可用空间约 23G。
- `campus-standalone-backend` 运行在 `127.0.0.1:18081`。
- `campus-standalone-frontend` 运行在 `0.0.0.0:18080`。
- `remote-smoke.ps1` 通过 `http://192.168.121.138:18080` 跑通：27 PASS / 0 FAIL / 0 SKIP。
- 138 上 standalone shell 脚本 `bash -n` 通过。

### 风险

- 当前 H2 smoke-only 栈不具备持久化数据能力。
- MySQL 镜像拉取仍受 138 网络影响。
- 正式发布仍依赖 owner 私有 release keystore / signing secrets。

### 下一轮建议

先确定 138 持久化 DB 路线：继续修复 Podman MySQL 镜像源，或由 owner 提供专用 MySQL 用户；之后再跑持久化部署 smoke 与 Android release 公网回归。

## Step 177 协作记录 - 三机集群环境巡检与低风险整理

### 本轮目标

owner 暂停应用部署，要求先梳理 `192.168.121.138 / 139 / 140` 三台机器的集群环境，并保护 Hive 上课环境。

### 实际改动

- 只读巡检三台基础资源、网络、时间同步、Java 进程、安装目录、关键端口和配置。
- 三台生成配置备份：`/root/cluster-config-backup-20260624-*.tar.gz`。
- 修正 `master` 上 ZooKeeper 三机 server 列表。
- 按 138 主控口径统一 SecondaryNameNode 到 `hbase01:50090`，随后 Step 178 已进一步改为 `master:50090`。

### 未改动内容

- 未启动、停止或重启 Hadoop / Hive / HBase / ZooKeeper / MySQL。
- 未删除 HDFS、ZooKeeper、MySQL 数据目录。
- 未重置 MySQL root。
- 未迁移 HDFS 数据目录。
...
- 三台当前 Java 进程均只有 `Jps`。
- HDFS 数据目录仍在 Hadoop 安装目录下。
- 只有 138 有 HBase 安装目录。
- 138 是 owner 主要操作入口。

## Step 178 协作记录 - 集群主机名规范化

### 本轮目标

将不合适的 `hbase01 / hbase02 / hbase03` 主机名规范化为正常节点名，并同步所有当前生效配置。

### 实际改动

- `192.168.121.138`：改为 `master`。
- `192.168.121.139`：改为 `worker01`。
- `192.168.121.140`：改为 `worker02`。
- 三台 `/etc/hosts` 改为新名优先，并保留旧名兼容别名。
- Hadoop 配置切换为 `master / worker01 / worker02`。
- ZooKeeper 配置切换为 `master / worker01 / worker02`。
- `master` 上 Hive JDBC 切换为 `jdbc:mysql://master:3306/metastore`。
- `master` 上 HBase 配置切换为 `hdfs://master:9000/hbase` 和 `hbase.zookeeper.quorum=master`。
- `worker01 / worker02` 的 chrony 注释切换为 `master`。

### 未改动内容

- 未删除 `/etc/hosts` 中的旧 `hbase01 / hbase02 / hbase03` 兼容别名。
- 未修改 LVM / GRUB / fstab 中的 `cs_hbase01`。
- 未修改 HDFS 数据块、Hive 历史作业文件、旧备份文件。
- 未启动、停止或重启任何集群服务。

### 验证结果

- 三台当前 hostname 分别为 `master / worker01 / worker02`。
- 当前生效配置除 `/etc/hosts` 兼容别名外无旧名残留。
- 三台 `jps -lm` 仍只有 `Jps`。

### 下一轮建议

基于新命名写 `master` 主控启动/停止 runbook 和只读健康检查脚本；HBase 暂不启动，除非先完成 `worker01 / worker02` 的 HBase 安装和配置同步。

## Step 179 协作记录 - 138/master Hive CLI 启动诊断

### 本轮目标

owner 澄清问题是“直接在 138 里面执行 hive”，因此本轮按 Hive CLI 口径重新复现，不再把 Beeline / HiveServer2 连接失败直接等同于 Hive CLI 不可用。

### 实际动作

- 连接 `192.168.121.138 / master`。
- 确认 `HIVE_HOME`、`HADOOP_HOME` 和 `hive` 命令路径。
- 执行 `hive -S -e "show databases;"`。

### 验证结果

- `hive` CLI 成功返回数据库列表。
- 138 上直接执行 `hive` 当前可用。
- `jdbc:hive2://master:10000` 仍应按 HiveServer2 是否启动单独判断。

### 未改动内容

- 未修改任何服务器配置。
- 未启动、停止或重启 Hadoop / Hive / HBase / ZooKeeper / MySQL。
- 未修改 Hive metastore、HDFS 数据目录或旧主机名兼容别名。

### 下一轮建议

如果课程只需要 `hive` CLI，先保持现状；如果需要 Beeline / JDBC，再启动 HiveServer2 并验证 `master:10000`。后续把这两类检查写入 `master` 主控健康检查脚本。

## Step 180 协作记录 - MySQL Flyway 正式化数据库改造

### 本轮目标

回到项目数据库上线前改造：保留 H2 作为 test / smoke fallback，但正式持久化部署切到 MySQL + Flyway。

### 实际改动

- 后端新增 Flyway 依赖。
- Maven 打包 `backend/db/migrations/V1..V14` 到 `classpath:db/migration`。
- `prod` profile 默认启用 Flyway。
- `test` profile 禁用 Flyway，继续使用 H2。
- 修正 V2 与 V3 重复字段问题。
- standalone MySQL 部署改成空库 + 后端 Flyway + 可选内测 seed。
- 新增 MySQL 内测 seed 文件，覆盖 smoke 所需关键账号和样本订单。

### 验证结果

- `./mvnw test`：58 tests / 0 failures / 0 errors。
- `./mvnw -q -DskipTests package`：通过。
- jar 内容检查：包含 `flyway-core-9.22.3.jar` 与 14 个迁移脚本。
- `git diff --check`：通过。
- `validate-samples.ps1`：0 hard failure，3 个既有可选样本 warning。

### 未改动内容

- 未连接或修改 138 的 Hive metastore MySQL。
- 未启动、停止或重启 Hadoop / Hive / HBase / ZooKeeper / MySQL。
- 未把 H2 从 test profile 删除。
- 未删除旧外卖模块或旧表。
...
## Step 183 协作记录 - 上线材料获取指南与 main 主线整理

### 本轮目标

owner 要求暂停其它操作，先整理一份“项目还差什么、怎么做、怎么申请、怎么获得”的文档，并整理项目文件和 `main` 主线口径。

### 实际动作

- 新增 `docs/deployment/launch-readiness-acquisition-guide.md`。
- 更新 `README.md`。
- 更新 `docs/README.md`。
- 更新 campus relay 项目日志。

### 结果

- 上线差距按 P0 / P1 / P2 梳理完成。
- 外部申请入口已集中到新文档。
- 项目文件整理口径与 `main` 分支状态口径已写入文档。
- 本轮没有服务器操作、数据库操作或业务代码变更。

### 下一轮建议

优先把文档里的“138 内测运维三件套”落地：备份、恢复演练、状态检查。

## Step 184 协作记录 - 仓库介绍去旧项目化

### 本轮目标

owner 要求修改仓库介绍，把“苍穹外卖”相关门面描述删掉。

### 实际动作

- 修改 README 项目介绍。
- 修改 docs 索引。
- 修改 API / DB / 项目状态 / 上线指南中的展示性旧项目口径。
- 新增 Step 184 日志。
...
如果 owner 想连 GitHub 仓库右侧 About 描述也一起改，需要单独确认仓库描述文案。
