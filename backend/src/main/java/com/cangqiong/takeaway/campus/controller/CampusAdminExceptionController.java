...
public class CampusAdminExceptionController {
    ...
    public Result<Void> resolve(@PathVariable Long id, @Valid @RequestBody CampusAdminExceptionResolveDTO dto) {
        Long employeeId = BaseContext.getCurrentUserId();
        log.info("校园代送异常历史处理关闭: employeeId={}, exceptionId={}", employeeId, id);
        campusExceptionRecordService.resolveByAdmin(id, dto, employeeId);
