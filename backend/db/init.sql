...
CREATE TABLE IF NOT EXISTS campus_settlement_reconcile_difference_record (
    ...
    FOREIGN KEY (settlement_record_id) REFERENCES campus_settlement_record(id),
    FOREIGN KEY (relay_order_id) REFERENCES campus_relay_order(id),
    FOREIGN KEY (courier_profile_id) REFERENCES campus_courier_profile(id),
    FOREIGN KEY (reported_by_employee_id) REFERENCES employee(id),
    FOREIGN KEY (processed_by_employee_id) REFERENCES employee(id)
...
CREATE TABLE IF NOT EXISTS campus_feedback (
    ...
    processed_by_employee_id BIGINT COMMENT '最近处理管理员ID',
    processed_at DATETIME COMMENT '最近处理时间',
